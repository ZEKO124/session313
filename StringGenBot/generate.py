...
ask_ques = "**»• ذا كنت تريد تنصب ميوزك اختار بايروجرام • واذا تريد تنصب تليثون فأختار تيرمڪس :**"
buttons_ques = [
    [
...
gen_button = [
    [
        InlineKeyboardButton(text="🔱اضغط لبدأ الاستخراج🔱", callback_data="generate")
    ]
]
...
async def generate_session(bot: Client, msg: Message, telethon=False, is_bot: bool = False):
    ...
    if is_bot:
        ty += " ʙᴏᴛ"
    await msg.reply(f"»   **{ty}** » ⚡️ ¦ بـدء إنـشـاء جـلسـة...")
    user_id = msg.chat.id
    api_id_msg = await bot.ask(user_id, "  **ᴀᴩɪ_ɪᴅ** 🎮حسنـا قم بأرسال الـ API_ID.\n\nᴄʟɪᴄᴋ ᴏɴ.", filters=filters.text)
    ...
    if api_id_msg.text == "/skip":
        ...
        try:
            ...
            return
        api_hash_msg = await bot.ask(user_id, "» **ᴀᴩɪ_ʜᴀsʜ** 🎮حسنـا قم بأرسال", filters=filters.text)
    ...
    try:
        phone_code_msg = None
        if not is_bot:
            phone_code_msg = await bot.ask(user_id, "»  ** ** يرجى ارسالها بالشكل التالي 1 2 3 4 5 مع وجود مسـافـات بين الارقام اذا ..\  ` `, ** ** `1 2 3 4 5`.", filters=filters.text, timeout=600)
    ...
    if not is_bot:
        ...
        try:
            ...
            try:
                ...
                await msg.reply("» تم انتهاء وقت الجلسه 10 دقائق يرجى اعاده استخراج الجلسه من البدايه ..\n\ .", reply_markup=InlineKeyboardMarkup(gen_button))
                return
            try:
    ...
    text = f"**ᴛتم استخراج ʀ {ty} جلسه** \n\n`{string_session}` \n\n**المطور :** @zeko124\n👨🏻‍💻 **ɴᴏᴛᴇ :**  لا تعطيه لأي شخص لان معرض للاختراق ❤️ @gibthon7 ᴛᴏ ᴊᴏɪɴ @gibthon7"
    try:
        if not is_bot:
